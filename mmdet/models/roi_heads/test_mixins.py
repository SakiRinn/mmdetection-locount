...
class BBoxTestMixinWithCount(BBoxTestMixin):
    ...
    if sys.version_info >= (3, 7):
        ...
        async def async_test_bboxes(self,
            ...
            async with completed(
                    __name__, 'bbox_head_forward',
                    sleep_interval=sleep_interval):
                cls_score, bbox_pred, cnt_score = self.bbox_head(roi_feats)

            img_shape = img_metas[0]['img_shape']
            scale_factor = img_metas[0]['scale_factor']
            det_bboxes, det_labels, det_counts = self.bbox_head.get_bboxes(
                rois,
                cls_score,
                bbox_pred,
                cnt_score,
    ...
    def simple_test_bboxes(self,
        ...
        if rois.shape[0] == 0:
            ...
            det_bbox = rois.new_zeros(0, 5)
            det_label = rois.new_zeros((0, ), dtype=torch.long)
            det_count = rois.new_zeros((0, ), dtype=torch.long)
            if rcnn_test_cfg is None:
                det_bbox = det_bbox[:, :4]
                det_label = rois.new_zeros(
                    (0, self.bbox_head.fc_cls.out_features))
                det_count = rois.new_zeros(
                    (0, self.bbox_head.fc_cnt.out_features))
            return [det_bbox] * batch_size, [det_label] * batch_size, [det_count] * batch_size
        ...
        for i in range(len(proposals)):
            if rois[i].shape[0] == 0:
                det_bbox = rois[i].new_zeros(0, 5)
                det_label = rois[i].new_zeros((0, ), dtype=torch.long)
                det_count = rois[i].new_zeros((0, ), dtype=torch.long)
                if rcnn_test_cfg is None:
                    det_bbox = det_bbox[:, :4]
                    det_label = rois[i].new_zeros(
                        (0, self.bbox_head.fc_cls.out_features))
                    det_count = rois[i].new_zeros(
                        (0, self.bbox_head.fc_cnt.out_features))
            else:
                det_bbox, det_label, det_count = self.bbox_head.get_bboxes(
                    rois[i],
                    cls_score[i],
                    bbox_pred[i],
                    cnt_score[i],
    ...
    def aug_test_bboxes(self, feats, img_metas, proposal_list, rcnn_test_cfg):
        ...
        for x, img_meta in zip(feats, img_metas):
            ...
            rois = bbox2roi([proposals])
            bbox_results = self._bbox_forward(x, rois)
            bboxes, scores, cnt_scores = self.bbox_head.get_bboxes(
                rois,
                bbox_results['cls_score'],
                bbox_results['bbox_pred'],
        ...
        merged_bboxes, merged_scores = merge_aug_bboxes(
            aug_bboxes, aug_scores, aug_cnt_scores, img_metas, rcnn_test_cfg)
        if merged_bboxes.shape[0] == 0:
            det_bboxes = merged_bboxes.new_zeros(0, 5)
            det_labels = merged_bboxes.new_zeros((0, ), dtype=torch.long)
            det_counts = merged_bboxes.new_zeros((0, ), dtype=torch.long)
        else:
