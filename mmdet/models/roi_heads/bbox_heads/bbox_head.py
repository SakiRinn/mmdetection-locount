...
class BBoxHeadWithCount(BBoxHead):
    ...
    def __init__(self,
                 ...
                 reg_predictor_cfg=dict(type='Linear'),
                 cls_predictor_cfg=dict(type='Linear'),
                 cnt_predictor_cfg=dict(type='Linear'),             # ADD
                 loss_cls=dict(
                     type='CrossEntropyLoss',
                     use_sigmoid=False,
                     loss_weight=1.0),
    ...
    def forward(self, x):
        ...
        cls_score = self.fc_cls(x) if self.with_cls else None
        bbox_pred = self.fc_reg(x) if self.with_reg else None
        cnt_score = self.fc_cnt(x) if self.with_cnt else None
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        if num_pos > 0:
            ...
            pos_weight = 1.0 if cfg.pos_weight <= 0 else cfg.pos_weight
            label_weights[:num_pos] = pos_weight
            count_weights[:num_pos] = pos_weight
            if not self.reg_decoded_bbox:
                pos_bbox_targets = self.bbox_coder.encode(
                    pos_bboxes, pos_gt_bboxes)
            else:
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = learning_bbox_weights * bbox_pred[pos_inds].sum()
        # cls
        if cls_score is not None:
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            if cls_score.numel() > 0:
