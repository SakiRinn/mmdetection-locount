...
@HEADS.register_module()
class FCBBoxHeadWithCount(ConvFCBBoxHead):
    r"""More general bbox head, with shared conv and fc layers and two optional
    ...
    def __init__(self,
        ...
        if 'with_cnt' in kwargs and isinstance(kwargs['with_cnt'], bool):
            ...
            del kwargs['with_cnt']
        if 'cnt_predictor_cfg' in kwargs and isinstance(kwargs['cnt_predictor_cfg'], dict):
            self.cnt_predictor_cfg = kwargs['cnt_predictor_cfg']
            del kwargs['cnt_predictor_cfg']
        if 'loss_cnt' in kwargs and isinstance(kwargs['loss_cnt'], dict):
    ...
    def forward(self, x):
        ...
        if x_cnt.dim() > 2:
            if self.with_avg_pool:
                x_cnt = self.avg_pool(x_cnt)
            x_cnt = x_cnt.flatten(1)
        for fc in self.cnt_fcs:
            x_cnt = self.relu(fc(x_cnt))
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        counts = pos_gt_counts

        return labels, label_weights, bbox_targets, bbox_weights, counts
    ...
    def get_targets(self,
        ...
        labels, label_weights, bbox_targets, bbox_weights, counts = multi_apply(
