...
@HEADS.register_module()
class DeformableDETRHeadWithCount(DETRHeadWithCount):
    ...
    def _init_layers(self):
        fc_cls = Linear(self.embed_dims, self.cls_out_channels)
        fc_cnt = Linear(self.embed_dims, self.cnt_out_channels)
        reg_branch = []
        for _ in range(self.num_reg_fcs):
            reg_branch.append(Linear(self.embed_dims, self.embed_dims))
            reg_branch.append(nn.ReLU())
        ...
        if self.with_box_refine:
            ...
            self.cls_branches = nn.ModuleList(
                [fc_cls for _ in range(num_pred)])
            self.cnt_branches = nn.ModuleList(
                [fc_cnt for _ in range(num_pred)])
            self.reg_branches = nn.ModuleList(
                [reg_branch for _ in range(num_pred)])
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for lvl in range(hs.shape[0]):
            ...
            reference = inverse_sigmoid(reference)
            outputs_class = self.cls_branches[lvl](hs[lvl])
            outputs_count = self.cnt_branches[lvl](hs[lvl])
            tmp = self.reg_branches[lvl](hs[lvl])
            if reference.shape[-1] == 4:
                tmp += reference
            else:
