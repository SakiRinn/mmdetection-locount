...
@HEADS.register_module()
class DDODHeadWithCount(AnchorHeadWithCount, DDODHead):
    ...
    def __init__(self,
        ...
        self.norm_cfg = norm_cfg
        self.use_dcn = use_dcn
        super(DDODHeadWithCount, self).__init__(num_classes, num_counts, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.cnt_num_pos_samples_per_level = [
            0. for _ in range(len(self.prior_generator.strides))
        ]
    ...
    def init_weights(self):
        ...
        for m in self.reg_convs:
            normal_init(m.conv, std=0.01)
        for m in self.cnt_convs:
            normal_init(m.conv, std=0.01)
        normal_init(self.atss_reg, std=0.01)
        normal_init(self.atss_iou, std=0.01)
        normal_init(self.atss_cls, std=0.01, bias=bias_init_with_prob(0.01))
        normal_init(self.atss_cnt, std=0.01, bias=bias_init_with_prob(0.01))
    ...
    def forward_single(self, x, scale):
        ...
        bbox_pred = scale(self.atss_reg(reg_feat)).float()
        iou_pred = self.atss_iou(reg_feat)
        cnt_score = self.atss_cnt(cnt_feat)
        return cls_score, bbox_pred, iou_pred, cnt_score

    def loss_cnt_single(self, cnt_score, counts, count_weights,
                        reweight_factor, num_total_samples):
        cnt_score = cnt_score.permute(0, 2, 3, 1).reshape(
            -1, self.cnt_out_channels).contiguous()
        ...
        return reweight_factor * loss_cnt,

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'iou_preds', 'cnt_scores'))
    def loss(self,
             cls_scores,
             bbox_preds,
             iou_preds,
        ...
        (anchor_list, valid_flag_list, num_level_anchors_list,
         cls_score_list, bbox_pred_list, cnt_score_list, gt_bboxes_ignore_list) = targets_com

        cls_cnt_targets = self.get_cls_targets(
            anchor_list,
            valid_flag_list,
            num_level_anchors_list,
            cls_score_list,
        ...
        if cls_cnt_targets is None:
            return None

        (cls_anchor_list, labels_list, label_weights_list,
         bbox_targets_list, bbox_weights_list, counts_list, count_weights_list,
         num_total_pos, num_total_neg) = cls_cnt_targets
        ...
        cls_losses, = multi_apply(
            ...
            num_total_samples=num_total_samples)
        # cnt
        cnt_losses, = multi_apply(
            self.loss_cnt_single,
            cnt_scores,
            counts_list,
            count_weights_list,
            reweight_factor_per_level,
        ...
        (reg_anchor_list, labels_list, label_weights_list,
         bbox_targets_list, bbox_weights_list, counts_list, count_weights_list,
         num_total_pos, num_total_neg) = reg_targets
    ...
    def process_predictions_and_anchors(self, anchor_list, valid_flag_list,
        ...
        mlvl_cnt_score_list = [
            cnt_score.permute(0, 2, 3, 1).reshape(
                num_imgs, -1, self.num_base_priors * self.cnt_out_channels)
            for cnt_score in cnt_scores
        ]
        ...
        for i in range(num_imgs):
            ...
            mlvl_cnt_tensor_list = [
                mlvl_cnt_score_list[j][i] for j in range(num_levels)
            ]
            cat_mlvl_cls_score = torch.cat(mlvl_cls_tensor_list, dim=0)
            cat_mlvl_bbox_pred = torch.cat(mlvl_bbox_tensor_list, dim=0)
            cat_mlvl_cnt_score = torch.cat(mlvl_cnt_tensor_list, dim=0)
            cls_score_list.append(cat_mlvl_cls_score)
            bbox_pred_list.append(cat_mlvl_bbox_pred)
            cnt_score_list.append(cat_mlvl_cnt_score)
        return (anchor_list_, valid_flag_list_, num_level_anchors_list,
                cls_score_list, bbox_pred_list, cnt_score_list, gt_bboxes_ignore_list)
    ...
    def get_cls_targets(self,
        ...
        (all_anchors, all_labels, all_label_weights,
         all_bbox_targets, all_bbox_weights, all_counts, all_count_weights,
        ...
        if any([labels is None for labels in all_labels]):
            return None
        if any([counts is None for counts in all_counts]):
            return None
        ...
        counts_list         = images_to_levels(all_counts, num_level_anchors_list[0])
        count_weights_list  = images_to_levels(all_count_weights, num_level_anchors_list[0])
        return (anchors_list, labels_list, label_weights_list,
                bbox_targets_list, bbox_weights_list, counts_list, count_weights_list,
    ...
    def _get_target_single(self,
        ...
        bbox_preds_valid = bbox_preds[inside_flags, :]
        cls_scores_valid = cls_scores[inside_flags, :]
        cnt_scores_valid = cnt_scores[inside_flags, :]

        assigner = self.cls_assigner if is_cls_assigner else self.reg_assigner

        bbox_preds_valid = self.bbox_coder.decode(anchors, bbox_preds_valid)
        assign_result = assigner.assign(anchors, num_level_anchors_inside,
                                        gt_bboxes, gt_bboxes_ignore, gt_labels, gt_counts,
                                        cls_scores_valid, bbox_preds_valid, cnt_scores_valid)
        sampling_result = self.sampler.sample(assign_result, anchors,
                                              gt_bboxes)
        ...
        if unmap_outputs:
            ...
            bbox_targets    = unmap(bbox_targets, num_total_anchors, inside_flags)
            bbox_weights    = unmap(bbox_weights, num_total_anchors, inside_flags)
            counts          = unmap(counts, num_total_anchors, inside_flags, fill=0)
            count_weights   = unmap(count_weights, num_total_anchors, inside_flags)
