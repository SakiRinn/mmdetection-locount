...
class BaseDenseHeadWithCount(BaseDenseHead, metaclass=ABCMeta):
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, cnt_score, score_factor, cnt_score_factor, priors) in \
            ...
            results = self._filter_scores_and_topk(
                ...
                dict(bbox_pred=bbox_pred, priors=priors))
            scores, labels, cnt_scores, counts, keep_idxs, filtered_results = results
        ...
        return self._bbox_post_process(mlvl_scores, mlvl_labels, mlvl_cnt_scores, mlvl_counts, mlvl_bboxes,
                                       img_meta['scale_factor'], cfg, rescale,
                                       with_nms, mlvl_score_factors, mlvl_cnt_score_factors, **kwargs)
    ...
    def _bbox_post_process(self,
                           ...
                           mlvl_cnt_score_factors=None,
                           **kwargs):
        assert len(mlvl_scores) == len(mlvl_bboxes) == len(mlvl_labels) == len(mlvl_cnt_scores) == len(mlvl_counts)
        ...
        if with_nms:
            if mlvl_bboxes.numel() == 0:
                det_bboxes = torch.cat([mlvl_bboxes, mlvl_scores[:, None], mlvl_cnt_scores[:, None]], -1)
                return det_bboxes, mlvl_labels, mlvl_counts

            det_bboxes, keep_idxs = batched_nms(mlvl_bboxes, mlvl_scores,
                                                mlvl_labels, cfg.nms)
            det_bboxes = torch.cat([det_bboxes, mlvl_cnt_scores], dim=0)
            det_bboxes = det_bboxes[:cfg.max_per_img]
            det_labels = mlvl_labels[keep_idxs][:cfg.max_per_img]
    ...
    def _filter_scores_and_topk(scores, cnt_scores, score_thr, topk, results=None):
        ...
        counts = torch.nonzero(valid_mask)
        ...
        return scores, labels, cnt_scores, counts, keep_idxs, filtered_results