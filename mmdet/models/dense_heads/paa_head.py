...
from mmdet.models.dense_heads import ATSSHead, ATSSHeadWithCount
...
@HEADS.register_module()
class PAAHeadWithCount(ATSSHeadWithCount, PAAHead):
    ...
    def __init__(self,
        ...
        self.with_score_voting = score_voting
        self.covariance_type = covariance_type
        super(PAAHeadWithCount, self).__init__(*args, **kwargs)

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'cnt_scores','iou_preds'))
    def loss(self,
             cls_scores,
             bbox_preds,
             cnt_scores,
        ...
        (labels, labels_weight, bboxes_target, bboxes_weight,
         counts, counts_weight, pos_inds, pos_gt_index) = cls_reg_cnt_targets
        cls_scores = levels_to_images(cls_scores)
        cls_scores = [item.reshape(-1, self.cls_out_channels) for item in cls_scores]
        bbox_preds = levels_to_images(bbox_preds)
        bbox_preds = [item.reshape(-1, 4) for item in bbox_preds]
        cnt_scores = levels_to_images(cnt_scores)
        cnt_scores = [item.reshape(-1, self.cnt_out_channels) for item in cnt_scores]
        iou_preds = levels_to_images(iou_preds)
        iou_preds = [item.reshape(-1, 1) for item in iou_preds]
        pos_losses_list, = multi_apply(self.get_pos_loss, anchor_list,
                                       cls_scores, bbox_preds, cnt_scores,
                                       labels, labels_weight, counts, counts_weight,
                                       bboxes_target, bboxes_weight, pos_inds)

        with torch.no_grad():
            reassign_labels, reassign_label_weight, reassign_counts, reassign_count_weight, \
                reassign_bbox_weights, num_pos = multi_apply(
                    self.paa_reassign,
                    pos_losses_list,
                    labels,
        ...
        cls_scores = torch.cat(cls_scores, 0).view(-1, cls_scores[0].size(-1))
        bbox_preds = torch.cat(bbox_preds, 0).view(-1, bbox_preds[0].size(-1))
        cnt_scores = torch.cat(cnt_scores, 0).view(-1, cnt_scores[0].size(-1))
        iou_preds = torch.cat(iou_preds, 0).view(-1, iou_preds[0].size(-1))
        ...
        labels = torch.cat(reassign_labels, 0).view(-1)
        labels_weight = torch.cat(reassign_label_weight, 0).view(-1)
        counts = torch.cat(reassign_counts, 0).view(-1)
        counts_weight = torch.cat(reassign_count_weight, 0).view(-1)
        ...
        return dict(
            ...
            loss_cnt=losses_cnt, loss_iou=losses_iou)

    def get_pos_loss(self, anchors, cls_score, bbox_pred, cnt_score,
                     label, label_weight, count, count_weight,
                     bbox_target, bbox_weight, pos_inds):
        if not len(pos_inds):
            return cls_score.new([]),
        anchors_all_level = torch.cat(anchors, 0)
        ...
        pos_scores = cls_score[pos_inds]
        pos_bbox_pred = bbox_pred[pos_inds]
        pos_cnt_scores = cnt_score[pos_inds]

        pos_label = label[pos_inds]
        pos_label_weight = label_weight[pos_inds]
        pos_count = count[pos_inds]
        pos_count_weight = count_weight[pos_inds]
        pos_bbox_target = bbox_target[pos_inds]
        pos_bbox_weight = bbox_weight[pos_inds]
        ...
        loss_cls = loss_cls.sum(-1) if loss_cls.ndim == 2 else loss_cls
        loss_cnt = loss_cnt.sum(-1) if loss_cnt.ndim == 2 else loss_cnt
        pos_loss = loss_bbox + loss_cls + loss_cnt
        return pos_loss,

    def paa_reassign(self, pos_losses, label, label_weight, count, count_weight,
                     bbox_weight, pos_inds, pos_gt_inds, anchors):
        if not len(pos_inds):
            return label, label_weight, count, count_weight, bbox_weight, 0
        label = label.clone()
        label_weight = label_weight.clone()
        count = count.clone()
        count_weight = count_weight.clone()
        bbox_weight = bbox_weight.clone()
        ...
        for gt_ind in range(num_gt):
            ...
            gmm = skm.GaussianMixture(
                ...
                precisions_init=precisions_init,
                covariance_type=self.covariance_type,
                reg_covar=1e-5)         # default: 1e-6. If default, it can't work.
            gmm.fit(pos_loss_gmm)
            gmm_assignment = gmm.predict(pos_loss_gmm)
            scores = gmm.score_samples(pos_loss_gmm)
            gmm_assignment = torch.from_numpy(gmm_assignment).to(device)
        ...
        label[reassign_ids] = self.num_classes      # label BG
        label_weight[ignore_inds_after_paa] = 0
        count[reassign_ids] = 0                     # count BG
        count_weight[ignore_inds_after_paa] = 0
        bbox_weight[reassign_ids] = 0
        num_pos = len(pos_inds_after_paa)
        return label, label_weight, count, count_weight, bbox_weight, num_pos
    ...
    def get_targets(
        ...
        (labels, label_weights, bbox_targets, bbox_weights, counts, count_weights,
         valid_pos_inds, valid_neg_inds, sampling_result) = results
        ...
        return (labels, label_weights, bbox_targets, bbox_weights,
                counts, count_weights, pos_inds, gt_inds)
    ...
    def _get_targets_single(self,
        ...
        assert unmap_outputs, 'We must map outputs back to the original' \
                              'set of anchors in PAAhead'
        return super(ATSSHeadWithCount, self)._get_targets_single(
            flat_anchors,
            valid_flags,
            gt_bboxes,
            gt_bboxes_ignore,
    ...
    def get_bboxes(self,
        ...
        assert with_nms, 'PAA only supports "with_nms=True" now and it ' \
                         'means PAAHead does not support ' \
                         'test-time augmentation'
        if score_factors is not None and cnt_score_factors is None:
            cnt_score_factors = score_factors
        return super(ATSSHeadWithCount, self).get_bboxes(cls_scores, bbox_preds, cnt_scores,
                                                         score_factors, cnt_score_factors,
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, cnt_score, score_factor, cnt_score_factor, priors) in \
            ...
            assert cls_score.size()[-2:] == bbox_pred.size()[-2:] == cnt_score.size()[-2:]

            scores = cls_score.permute(1, 2, 0).reshape(
                -1, self.cls_out_channels).sigmoid()
            cnt_scores = cnt_score.permute(1, 2, 0).reshape(
                -1, self.cnt_out_channels).sigmoid()
            bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 4)
            score_factor = score_factor.permute(1, 2, 0).reshape(-1).sigmoid()
            cnt_score_factor = cnt_score_factor.permute(1, 2, 0).reshape(-1).sigmoid()
            ...
            if 0 < nms_pre < scores.shape[0]:
                ...
                score_factor     = score_factor[topk_inds]
                cnt_score_factor = cnt_score_factor[topk_inds]
        ...
        return self._bbox_post_process(mlvl_scores, mlvl_cnt_scores, mlvl_bboxes,
                                       img_meta['scale_factor'], None, cfg, rescale, with_nms,
                                       mlvl_score_factors, mlvl_cnt_score_factors, **kwargs)
    ...
    def _bbox_post_process(self,
        ...
        # Add a dummy BG (idx=num_classes) to the backend when using sigmoid.
        padding = mlvl_scores.new_zeros(mlvl_scores.shape[0], 1)
        mlvl_scores = torch.cat([mlvl_scores, padding], dim=1)

        mlvl_iou_preds = torch.cat(mlvl_score_factors)
        mlvl_nms_scores = (mlvl_scores * mlvl_iou_preds[:, None]).sqrt()
        # mlvl_nms_cnt_scores = (mlvl_cnt_scores * torch.cat(mlvl_cnt_score_factors)[:, None]).sqrt()
        ...
        cnt_scores, counts = torch.max(mlvl_cnt_scores[..., 1:], dim=-1)
        cnt_scores, counts = cnt_scores[..., None], (counts + 1)[..., None]
        ...
        if self.with_score_voting and len(det_bboxes) > 0:
            det_bboxes, det_labels = self.score_voting(det_bboxes, det_labels,
                                                       mlvl_bboxes, mlvl_nms_scores, cfg.score_thr)
