...
@HEADS.register_module()
class YOLOFHeadWithCount(AnchorHeadWithCount, YOLOFHead):
    ...
    def __init__(self,
        ...
        self.norm_cfg = norm_cfg
        super(YOLOFHeadWithCount, self).__init__(num_classes, num_counts, in_channels,
                                                 **kwargs)
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_reg_convs):
            bbox_subnet.append(
                ConvModule(
                    ...
                    padding=1,
                    norm_cfg=self.norm_cfg))
        for i in range(self.num_cnt_convs):
            cnt_subnet.append(
                ConvModule(
                    self.in_channels,
                    self.in_channels,
        ...
        self.cls_subnet = nn.Sequential(*cls_subnet)
        self.bbox_subnet = nn.Sequential(*bbox_subnet)
        self.cnt_subnet = nn.Sequential(*cnt_subnet)

        self.cls_score = nn.Conv2d(
            self.in_channels,
            self.num_base_priors * self.cls_out_channels,       # XXX: num_classes -> cls_out_channels
            kernel_size=3,
            stride=1,
            padding=1)
        self.bbox_pred = nn.Conv2d(
    ...
    def init_weights(self):
        ...
        bias_cls = bias_init_with_prob(0.01)
        torch.nn.init.constant_(self.cls_score.bias, bias_cls)
        bias_cnt = bias_init_with_prob(0.01)
        torch.nn.init.constant_(self.cnt_score.bias, bias_cnt)

    def forward_single(self, feature):
        # cls
        cls_score = self.cls_score(self.cls_subnet(feature))
        N, _, H, W = cls_score.shape
        cls_score = cls_score.view(N, -1, self.cls_out_channels, H, W)      # XXX: num_classes -> cls_out_channels
        # cnt
        cnt_score = self.cnt_score(self.cnt_subnet(feature))
        cnt_score = cnt_score.view(N, -1, self.cnt_out_channels, H, W)
        # bbox
        reg_feat = self.bbox_subnet(feature)
        bbox_reg = self.bbox_pred(reg_feat)
        objectness = self.object_pred(reg_feat)
        ...
        normalized_cls_score = normalized_cls_score.view(N, -1, H, W)
        # (2) norm_cnt
        normalized_cnt_score = cnt_score + objectness - torch.log(
            1. + torch.clamp(cnt_score.exp(), max=INF) +
            torch.clamp(objectness.exp(), max=INF))
        normalized_cnt_score = normalized_cnt_score.view(N, -1, H, W)

        return normalized_cls_score, bbox_reg, normalized_cnt_score
    ...
    def loss(self,
             ...
             img_metas,
             gt_bboxes_ignore=None):
        assert len(cls_scores) == len(cnt_scores) == 1
        assert self.prior_generator.num_levels == 1
        device = cls_scores[0].device
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores]
        ...
        cls_scores_list = levels_to_images(cls_scores)
        bbox_preds_list = levels_to_images(bbox_preds)
        cnt_scores_list = levels_to_images(cnt_scores)
        ...
        if cls_reg_cnt_targets is None:
            return None
        (batch_labels, batch_label_weights, batch_counts, batch_count_weights,
         num_total_pos, num_total_neg,
         batch_bbox_weights, batch_pos_predicted_boxes, batch_target_boxes) = cls_reg_cnt_targets
        ...
        cls_score = cls_scores[0].permute(0, 2, 3,
                                          1).reshape(-1, self.cls_out_channels)
        # cnt
        flatten_counts = batch_counts.reshape(-1)
        batch_count_weights = batch_count_weights.reshape(-1)
        cnt_score = cnt_scores[0].permute(0, 2, 3,
        ...
        return dict(loss_cls=loss_cls, loss_bbox=loss_bbox, loss_cnt=loss_cnt)
    ...
    def get_targets(self,
        ...
        results = multi_apply(
            ...
            count_channels=count_channels,
            unmap_outputs=unmap_outputs)
        (all_labels, all_label_weights, all_counts, all_count_weights,
        ...
        batch_labels = torch.stack(all_labels, 0)
        batch_label_weights = torch.stack(all_label_weights, 0)
        batch_counts = torch.stack(all_counts, 0)
        batch_count_weights = torch.stack(all_count_weights, 0)

        res = (batch_labels, batch_label_weights,
               batch_counts, batch_count_weights,
               num_total_pos, num_total_neg)
    ...
    def _get_targets_single(self,
        ...
        return (labels, label_weights, counts, count_weights,
