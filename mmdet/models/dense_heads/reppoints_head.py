...
@HEADS.register_module()
class RepPointsHeadWithCount(AnchorFreeHeadWithCount, RepPointsHead):
    ...
    def __init__(self,
        ...
        if self.use_sigmoid_cnt:
            self.cnt_out_channels = self.num_counts
        else:
            self.cnt_out_channels = self.num_counts + 1
    ...
    def forward_single(self, x):
        ...
        cls_out = self.reppoints_cls_out(
            self.relu(self.reppoints_cls_conv(cls_feat, dcn_offset)))
        cnt_out = self.reppoints_cnt_out(
            self.relu(self.reppoints_cnt_conv(cnt_feat, dcn_offset)))
        pts_out_refine = self.reppoints_pts_refine_out(
            self.relu(self.reppoints_pts_refine_conv(pts_feat, dcn_offset)))
        if self.use_grid_points:
            pts_out_refine, bbox_out_refine = self.gen_grid_from_reg(
    ...
    def _point_target_single(self,
        ...
        if unmap_outputs:
            ...
            counts              = unmap(counts, num_total_proposals, inside_flags)
            count_weights       = unmap(count_weights, num_total_proposals, inside_flags)
            bbox_gt             = unmap(bbox_gt, num_total_proposals, inside_flags)
    ...
    def get_targets(self,
        ...
        counts_list             = images_to_levels(all_counts, num_level_proposals)
        count_weights_list      = images_to_levels(all_count_weights, num_level_proposals)
        bbox_gt_list            = images_to_levels(all_bbox_gt, num_level_proposals)
        proposals_list          = images_to_levels(all_proposals, num_level_proposals)
    ...
    def loss(self,
        ...
        cls_reg_cnt_targets_init = self.get_targets(
            candidate_list,
            valid_flag_list,
            gt_bboxes,
            img_metas,
        ...
        for i_img, center in enumerate(center_list):
            ...
            for i_lvl in range(len(pts_preds_refine)):
                ...
                bbox.append(bbox_center +
                            bbox_shift[i_img].permute(1, 2, 0).reshape(-1, 4))
            bbox_list.append(bbox)
        cls_reg_cnt_targets_refine = self.get_targets(
            bbox_list,
            valid_flag_list,
            gt_bboxes,
            img_metas,
    ...
    def _get_bboxes_single(self,
                           ...
                           **kwargs):
        cfg = self.test_cfg if cfg is None else cfg
        assert len(cls_score_list) == len(bbox_pred_list) == len(cnt_score_list)
        img_shape = img_meta['img_shape']
        nms_pre = cfg.get('nms_pre', -1)
        ...
        mlvl_cnt_scores = []
        mlvl_counts = []
        for level_idx, (cls_score, bbox_pred, cnt_score, priors) in enumerate(
                zip(cls_score_list, bbox_pred_list, cnt_score_list, mlvl_priors)):

            assert cls_score.size()[-2:] == bbox_pred.size()[-2:] == cnt_score.size()[-2:]
            bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 4)
            cls_score = cls_score.permute(1, 2, 0).reshape(-1, self.cls_out_channels)
            cnt_score = cnt_score.permute(1, 2, 0).reshape(-1, self.cnt_out_channels)
