...
@HEADS.register_module()
class DETRHeadWithCount(AnchorFreeHeadWithCount, DETRHead):
    ...
    def __init__(self,
                 ...
                 train_cfg=dict(
                     assigner=dict(
                         ...
                         cnt_cost=dict(type='ClassificationCost', weight=1.0),
                         reg_cost=dict(type='BBoxL1Cost', weight=5.0),
        ...
        self.bg_cls_weight = 0
        self.bg_cnt_weight = 0
        self.sync_cls_avg_factor = sync_cls_avg_factor
        self.sync_cnt_avg_factor = sync_cnt_avg_factor

        # cls weight
        class_weight = loss_cls.get('class_weight', None)
        if class_weight is not None and (self.__class__ is DETRHeadWithCount):
            assert isinstance(class_weight, float), 'Expected ' \
                'class_weight to have type float. Found ' \
                f'{type(class_weight)}.'
            bg_cls_weight = loss_cls.get('bg_cls_weight', class_weight)
            ...
            self.bg_cls_weight = bg_cls_weight
        # cnt weight
        count_weight = loss_cnt.get('count_weight', None)
        if count_weight is not None and (self.__class__ is DETRHeadWithCount):
            assert isinstance(count_weight, float), 'Expected ' \
                'count_weight to have type float. Found ' \
                f'{type(count_weight)}.'
            bg_cnt_weight = loss_cnt.get('bg_cnt_weight', count_weight)
            assert isinstance(bg_cnt_weight, float), 'Expected ' \
                'bg_cnt_weight to have type float. Found ' \
                f'{type(bg_cnt_weight)}.'
            count_weight = torch.ones(num_counts + 1) * count_weight
            count_weight[num_counts] = bg_cnt_weight
            loss_cnt.update({'count_weight': count_weight})
            if 'bg_cnt_weight' in loss_cnt:
        ...
        if train_cfg:
            ...
            assert loss_iou['loss_weight'] == assigner['iou_cost']['weight'], \
                'The regression iou weight for loss and matcher should be' \
                'exactly the same.'
            assert loss_cnt['loss_weight'] == assigner['cnt_cost']['weight'], \
                'The count weight for loss and matcher should be exactly the same.'
            self.assigner = build_assigner(assigner)
    ...
    def _init_layers(self):
        self.input_proj = Conv2d(
            self.in_channels, self.embed_dims, kernel_size=1)
        self.fc_cls = Linear(self.embed_dims, self.cls_out_channels)
        self.fc_cnt = Linear(self.embed_dims, self.cnt_out_channels)
        self.reg_ffn = FFN(
            self.embed_dims,
            self.embed_dims,
            self.num_reg_fcs,
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
                              missing_keys, unexpected_keys, error_msgs):
        version = local_metadata.get('version', None)
        if (version is None or version < 2) and self.__class__ is DETRHeadWithCount:
            convert_dict = {
                '.self_attn.': '.attentions.0.',
                '.ffn.': '.ffns.0.',
                '.multihead_attn.': '.attentions.1.',
    ...
    def forward_single(self, x, img_metas):
        ...
        all_cls_scores = self.fc_cls(outs_dec)
        all_cnt_scores = self.fc_cnt(outs_dec)
        all_bbox_preds = self.fc_reg(self.activate(
            self.reg_ffn(outs_dec))).sigmoid()
        return all_cls_scores, all_bbox_preds, all_cnt_scores
    ...
    def loss(self,
        ...
        all_cls_scores = all_cls_scores_list[-1]
        all_bbox_preds = all_bbox_preds_list[-1]
        all_cnt_scores = all_cnt_scores_list[-1]
        assert gt_bboxes_ignore is None, \
            'Only supports for gt_bboxes_ignore setting to None.'
        ...
        all_gt_bboxes_list = [gt_bboxes_list for _ in range(num_dec_layers)]
        all_gt_labels_list = [gt_labels_list for _ in range(num_dec_layers)]
        all_gt_counts_list = [gt_counts_list for _ in range(num_dec_layers)]
        all_gt_bboxes_ignore_list = [
            gt_bboxes_ignore for _ in range(num_dec_layers)
        ]
        img_metas_list = [img_metas for _ in range(num_dec_layers)]

        losses_cls, losses_bbox, losses_iou, losses_cnt = multi_apply(
            self.loss_single,
            all_cls_scores, all_bbox_preds, all_cnt_scores,
            all_gt_bboxes_list, all_gt_labels_list, all_gt_counts_list,
            img_metas_list, all_gt_bboxes_ignore_list)

        loss_dict = dict()
        # (1) Last decoder layer
        loss_dict['loss_cls'] = losses_cls[-1]
        loss_dict['loss_bbox'] = losses_bbox[-1]
        loss_dict['loss_iou'] = losses_iou[-1]
        loss_dict['loss_cnt'] = losses_cnt[-1]
        # (2) Other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i, loss_cnt_i in zip(
                losses_cls[:-1], losses_bbox[:-1], losses_iou[:-1], losses_cnt[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
            loss_dict[f'd{num_dec_layer}.loss_cnt'] = loss_cnt_i
            num_dec_layer += 1
        return loss_dict
    ...
    def loss_single(self,
        ...
        cls_scores_list = [cls_scores[i] for i in range(num_imgs)]
        bbox_preds_list = [bbox_preds[i] for i in range(num_imgs)]
        cnt_scores_list = [cnt_scores[i] for i in range(num_imgs)]
        cls_reg_cnt_targets = self.get_targets(cls_scores_list, bbox_preds_list, cnt_scores_list,
                                               gt_bboxes_list, gt_labels_list, gt_counts_list,
                                               img_metas, gt_bboxes_ignore_list)
        ...
        bbox_targets = torch.cat(bbox_targets_list, 0)
        bbox_weights = torch.cat(bbox_weights_list, 0)
        counts = torch.cat(counts_list, 0)
        count_weights = torch.cat(count_weights_list, 0)

        # 1. classification loss (cls/cnt)
        cls_scores = cls_scores.reshape(-1, self.cls_out_channels)
        cnt_scores = cnt_scores.reshape(-1, self.cnt_out_channels)
        ...
        cls_avg_factor = max(cls_avg_factor, 1)
        # (2) cnt
        cnt_avg_factor = num_total_pos * 1.0 + \
            num_total_neg * self.bg_cnt_weight
        if self.sync_cnt_avg_factor:
            cnt_avg_factor = reduce_mean(
                cnt_scores.new_tensor([cnt_avg_factor]))
        ...
        loss_cnt = self.loss_cnt(
            cnt_scores, counts, count_weights, avg_factor=cnt_avg_factor)

        # average number of gt boxes
        num_total_pos = loss_cls.new_tensor([num_total_pos])
        num_total_pos = torch.clamp(reduce_mean(num_total_pos), min=1).item()
        ...
        # 2. regression IoU loss
        loss_iou = self.loss_iou(
            bboxes, bboxes_gt, bbox_weights, avg_factor=num_total_pos)

        # 3. regression L1 loss
        loss_bbox = self.loss_bbox(
            bbox_preds, bbox_targets, bbox_weights, avg_factor=num_total_pos)
        return loss_cls, loss_bbox, loss_iou, loss_cnt
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list,
         ...
         pos_inds_list, neg_inds_list) = multi_apply(
             ...
             cls_scores_list, bbox_preds_list, cnt_scores_list,
             gt_bboxes_list, gt_labels_list, gt_counts_list,
             img_metas, gt_bboxes_ignore_list)
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))
    ...
    def _get_target_single(self,
        ...
        num_bboxes = bbox_pred.size(0)
        # assigner and sampler
        assign_result = self.assigner.assign(bbox_pred, cls_score, cnt_score,
        ...
        # count targets
        counts = gt_bboxes.new_full((num_bboxes, ),
                                    0,
                                    dtype=torch.long)
        counts[pos_inds] = gt_counts[sampling_result.pos_assigned_gt_inds]
        count_weights = gt_bboxes.new_ones(num_bboxes)
    ...
    def get_bboxes(self,
        ...
        cls_scores = all_cls_scores_list[-1][-1]
        bbox_preds = all_bbox_preds_list[-1][-1]
        cnt_scores = all_cnt_scores_list[-1][-1]

        result_list = []
        for img_id in range(len(img_metas)):
            cls_score = cls_scores[img_id]
            bbox_pred = bbox_preds[img_id]
            cnt_score = cnt_scores[img_id]
            img_shape = img_metas[img_id]['img_shape']
            scale_factor = img_metas[img_id]['scale_factor']
            proposals = self._get_bboxes_single(cls_score, bbox_pred, cnt_score,
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            scores, det_labels = F.softmax(cls_score, dim=-1)[..., :-1].max(-1)         # exclude BG
            scores, bbox_index = scores.topk(max_per_img)
            bbox_pred = bbox_pred[bbox_index]
            det_labels = det_labels[bbox_index]
        # cnt
