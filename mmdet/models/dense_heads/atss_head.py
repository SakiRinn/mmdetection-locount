...
@HEADS.register_module()
class ATSSHeadWithCount(AnchorHeadWithCount, ATSSHead):
    ...
    def __init__(self,
        ...
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        super(ATSSHeadWithCount, self).__init__(
            num_classes,
            num_counts,
            in_channels,
            reg_decoded_bbox=reg_decoded_bbox,
    ...
    def _init_layers(self):
        ...
        self.atss_cnt = nn.Conv2d(
            self.feat_channels,
            self.num_anchors * self.cnt_out_channels,
            self.pred_kernel_size,
            padding=pred_pad_size)
        self.atss_centerness = nn.Conv2d(
            self.feat_channels,
    ...
    def forward_single(self, x, scale):
        ...
        centerness = self.atss_centerness(reg_feat)
        return cls_score, bbox_pred, cnt_score, centerness

    def loss_single(self, anchors, cls_score, bbox_pred, cnt_score, centerness,
                    labels, label_weights, counts, count_weights,
        ...
        return loss_cls, loss_bbox, loss_cnt, loss_centerness, centerness_targets.sum()

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'cnt_scores', 'centernesses'))
    def loss(self,
             cls_scores,
             bbox_preds,
             cnt_scores,
        ...
        losses_cls, losses_bbox, losses_cnt, loss_centerness, \
            bbox_avg_factor = multi_apply(
                self.loss_single,
                anchor_list,
                cls_scores,
