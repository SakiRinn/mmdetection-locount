...
class MaxIoUAssignerWithCount(MaxIoUAssigner, BaseAssignerWithCount):
    ...
    def __init__(self,
                 ...
                 gpu_assign_thr=-1,
                 iou_calculator=dict(type='BboxOverlaps2D')):
        super(MaxIoUAssignerWithCount, self).__init__(
            pos_iou_thr,
            neg_iou_thr,
            min_pos_iou,
            gt_max_assign_all,
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None, gt_counts=None):
        assign_on_cpu = True if (self.gpu_assign_thr > 0) and (
            gt_bboxes.shape[0] > self.gpu_assign_thr) else False
        if assign_on_cpu:
            device = bboxes.device
        ...
        if assign_on_cpu:
            ...
            if assign_result.counts is not None:
                assign_result.counts = assign_result.counts.to(device)
        return assign_result

    def assign_wrt_overlaps(self, overlaps, gt_labels=None, gt_counts=None):
        num_gts, num_bboxes = overlaps.size(0), overlaps.size(1)
        ...
        return AssignResultWithCount(
            num_gts, assigned_gt_inds, max_overlaps, labels=assigned_labels, counts=assigned_counts)