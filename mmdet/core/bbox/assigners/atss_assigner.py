...
@BBOX_ASSIGNERS.register_module()
class ATSSAssignerWithCount(BaseAssignerWithCount):
    ...
    def assign(self,
        ...
        message = 'Invalid alpha parameter because cls_scores or ' \
                  'bbox_preds or cnt_scores are None. If you want to use the ' \
                  'cost-based ATSSAssigner,  please set cls_scores, ' \
                  'bbox_preds, cnt_scores and self.alpha at the same time. '

        if self.alpha is None:
            overlaps = self.iou_calculator(bboxes, gt_bboxes)
            if cls_scores is not None or bbox_preds is not None or cnt_scores is not None:
                warnings.warn(message)
        else:
            assert cls_scores is not None and bbox_preds is not None and cnt_scores is not None, \
                   message

            cls_cost = torch.sigmoid(cls_scores[:, gt_labels])
            # cnt_cost = torch.sigmoid(cnt_scores[:, gt_counts])
            overlaps = self.iou_calculator(bbox_preds, gt_bboxes)

            # assert cls_cost.shape == cnt_cost.shape == overlaps.shape
            # overlaps = (cls_cost + cnt_cost)**(1 - self.alpha) * overlaps**self.alpha
            assert cls_cost.shape == overlaps.shape
            overlaps = cls_cost**(1 - self.alpha) * overlaps**self.alpha
        ...
        if num_gt == 0 or num_bboxes == 0:
            ...
            if gt_counts is None:
                ...
                assigned_counts = overlaps.new_full((num_bboxes, ),
                                                    0,
                                                    dtype=torch.long)
        ...
        if gt_counts is not None:
            ...
            pos_inds = torch.nonzero(
                assigned_gt_inds > 0, as_tuple=False).squeeze()
            if pos_inds.numel() > 0:
                assigned_counts[pos_inds] = gt_counts[
                    assigned_gt_inds[pos_inds] - 1]
        else:
            assigned_counts = None
        return AssignResultWithCount(
